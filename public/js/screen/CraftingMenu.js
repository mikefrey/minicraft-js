...
function CraftingMenu(recipes, player) {
  ...
  this.recipes = this.recipes.sort(function(r1, r2) {
...
CraftingMenu.prototype = extend(new Menu(), {
  ...
  tick: function() {
    ...
    if (input.attack.clicked && len > 0) {
      var r = this.recipes[this.selected];
  ...
  render: function(screen) {
    ...
    if (this.recipes.length > 0) {
      ...
      var hasResultItems = this.player.inventory.count(recipe.resultTemplate);
      var xo = 13 * 8;
      ...
      for (var i = 0; i < costs.length; i++) {
        ...
        var has = this.player.inventory.count(item);
