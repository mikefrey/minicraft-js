...
class CloudTile extends Tile {
  ...
  render(screen, level, x, y) {
    const col = Color.get(444, 444, 555, 555)
    const transitionColor = Color.get(333, 444, 555, -1)

    const u = level.getTile(x, y - 1) == Tile.infiniteFall
    const d = level.getTile(x, y + 1) == Tile.infiniteFall
    const l = level.getTile(x - 1, y) == Tile.infiniteFall
    const r = level.getTile(x + 1, y) == Tile.infiniteFall

    const ul = level.getTile(x - 1, y - 1) == Tile.infiniteFall
    const dl = level.getTile(x - 1, y + 1) == Tile.infiniteFall
    const ur = level.getTile(x + 1, y - 1) == Tile.infiniteFall
    const dr = level.getTile(x + 1, y + 1) == Tile.infiniteFall
