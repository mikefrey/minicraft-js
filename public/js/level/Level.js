...
function Level(w, h, level, parentLevel) {
...
Level.prototype = {
  ...
  renderLight: function(screen, xScroll, yScroll) {
    var xo = xScroll >> 4;
    var yo = yScroll >> 4;
    var w = (screen.w + 15) >> 4;
    var h = (screen.h + 15) >> 4;
    var r = 4;
    var y, x, i, entities, e, lr;
  ...
  sortAndRender: function(screen, list) {
    var sorted = list.sort(this.spriteSorter);
  ...
  setTile: function(x, y, t, dataVal) {
  ...
  getEntities: function(x0, y0, x1, y1) {
    var result = [];
    var xt0 = (x0 >> 4) - 1;
    var yt0 = (y0 >> 4) - 1;
    var xt1 = (x1 >> 4) + 1;
    ...
    for (var y = yt0; y <= yt1; y++) {
      for (var x = xt0; x <= xt1; x++) {
        if (x < 0 || y < 0 || x >= this.w || y >= this.h) continue;
        var entities = this.entitiesInTiles[x + y * this.w];
